...
<div class="container">
	...
	<c:forEach var="reply1" items="${replyl1s}" varStatus="status">
		<c:if test="${null ne reply1.REPLY_NUM1 && reply1.REPLY_TYPE2 == rboard.ID}">
	...
	<div class="row d-flex justify-content-between">
		<div class="row">
		<div class="ml-5"style="margin-top:3px;">
			<h4>${rboard.RBOARDTITLE}</h4>
		</div>
		...
		<div class="">
			<form>
				<input id="title" type="hidden" value="${rboard.RBOARDTITLE}${rboard.ID}${principal.user.username}"/>
	...
	<div class="mb-5">
		글 번호 : <span id="id"><i>${rboard.ID} </i></span> 작성자 : <span><i> ${rboard.RBOARDUSERNAME}  </i></span>&nbsp;<span>작성일: ${rboard.RBOARDCDATE}</span> <br />
	</div>
	...
	<div class="d-flex justify-content-center">
		<div class="slick-sample d-flex justify-content-center mt-4" style="text-align: center;">
			<div class="col mb-5">
				...
				<div style="margin-top: 100px;" class="card-body">
					${rboard.RBOARD01}
				</div>
			</div>
			<div class="col">
				...
				<div style="margin-top: 100px;" class="card-body mb-5">
					${rboard.RBOARD02}
				</div>
			</div>
	...
	<div class="form-group" style="min-height: 300px;">
		<div class="mt-3 mb-3">${rboard.RBOARDCONTENT}</div>
	</div>

	<p>메뉴명 : -${rboard.RBOARDNM}-</p>
	<p>분류 : -${rboard.RBOARDFTYPE}-</p>
	<hr />



	<c:if test="${rboard.RBOARDUSERNAME==principal.user.username}">
		<div class="d-flex justify-content-end mb-2" style="margin-top: -5px;">
			<a href="/rboard/${rboard.ID}/RupdateForm" class="bnon">
			...
			<button id="btn-rdelete" class="bnon">
	...
	<form>
		<div class="d-flex justify-content-center">
			<input id="ID1" type="hidden" value="2"/>
			<input id="ID2" type="hidden" value="${rboard.ID}"/>
	...
	<div class="card">
		...
		<c:forEach var="reply" items="${replyl1s}">
			<c:if test="${reply.REPLY_TYPE2 == rboard.ID && reply.REPLY_TYPE == 2 && principal.user.username eq reply.REPLY_USERNAME}">
				<div class="m-3 ml-5"
					...
					<div class="d-flex justify-content-between">
						...
						<button onClick="index.RreplyDelete(${rboard.ID}, ${reply.id})" class="bnon pr-2 pl-2" style="background-color: rgba(0,0,0,0)">
			...
			<c:if test="${reply.REPLY_TYPE2 == rboard.ID && principal.user.username ne reply.REPLY_USERNAME}">
				<div class=" d-flex justify-content-end" style="width:100%;">
					<div class="m-3 mr-5" style="background-color: rgba(87, 159, 251, 0.8); width:600px; min-height: 100px; border-radius: 18px 18px 0px 18px;">
						...
						<div class="d-flex justify-content-between">
							...
							<button onClick="index.RreplyDelete(${rboard.ID}, ${reply.id})" class="btn pr-2 pl-2">
								<svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor"
									 class="bi bi-trash2-fill" viewBox="0 0 16 16">
									<path d="M2.037 3.225A.703.703 0 0 1 2 3c0-1.105 2.686-2 6-2s6 .895 6 2a.702.702 0 0 1-.037.225l-1.684 10.104A2 2 0 0 1 10.305 15H5.694a2 2 0 0 1-1.973-1.671L2.037 3.225zm9.89-.69C10.966 2.214 9.578 2 8 2c-1.58 0-2.968.215-3.926.534-.477.16-.795.327-.975.466.18.14.498.307.975.466C5.032 3.786 6.42 4 8 4s2.967-.215 3.926-.534c.477-.16.795-.327.975-.466-.18-.14-.498-.307-.975-.466z"/>
								</svg>
		...
		<script>
			function work()  {
				const target = document.getElementById('btn-reply-save');
