...
public class DummyControllerTest {
	...
	public User updateUser(@PathVariable int id,@RequestBody User requestUser) { //json 데이터를 요청 => java object (messageConverter의 jackson 라이브러리가 변환해서 받아줌
		...
		System.out.println("password:"+requestUser.getUSER_PASSWORD());
	    System.out.println("email"+requestUser.getUSER_EMAIL());
//findById이아이디를 찾을수없으면 orElseThrow 특정함수를 사용해라 28강 10:18
	    
	    User user = userRepository.findById(id).orElseThrow(()->{
	    	return new IllegalArgumentException("수정에 실패하였습니다.");
	    });
	    user.setUSER_PASSWORD(requestUser.getUSER_PASSWORD());
	    user.setUSER_EMAIL(requestUser.getUSER_EMAIL());
	...
	public String join(User user) { // key=value(약속된 규칙)
	  ...
	  System.out.println("createDate:"+user.getUSER_CDATE());
